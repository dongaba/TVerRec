...
function readStorage() {
	chrome.storage.local.get(null, (data) => {
		...
		if ((key_uid && key_token) || (key_sid)) {
			...
			if (uidElement) {uidElement.innerText = `platform_uid : ${key_uid}`;}
			if (tokenElement) {tokenElement.innerText = `platform_token : ${key_token}`;}
			if (sidElement) {sidElement.innerText = `member_sid : ${key_sid}`;}
		}
	});
}
...
document.getElementById("refresh").addEventListener("click", async () => {
	clearStorage();
	console.log("TVerRec Assistant: platform_uid, platform_token and member_sid cleared");
