...
7. ダウンロードされたファイルは、最終保存先に**自動的に整理**可能です。
    - 例えば毎週同じ番組をダウンロードする場合、最終保存先に番組名のフォルダがあれば自動的に全番組が最終保存先のフォルダに移動されます。
    - 最終保存先に同名のフォルダがなければ、動画ファイルは保存先フォルダに残り続けます。
8. 動作に必要なyt-dlpやffmpegなどの必要コンポーネントは**自動的に最新版がダウンロード**されます。(Windowsのみ)
9. **日本国外からもVPN不要**で利用することができます。
...
- ちゃんと検証していませんが、なんとなくMacだとファイルアクセスが遅い気がします。
