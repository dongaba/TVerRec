...
function saveGenrePage ($script:KeywordName) {
	...
	$local:keywordFile = $(Join-Path $script:ugDir (getFileNameWithoutInvalidChars $local:keywordFile))
...
function getIgnoreList {
	#ダウンロード対象外番組リストの読み込み
	try {
		$local:ignoreTitles = (Get-Content $script:oreFilePath -Encoding UTF8 `
...
function loadKeywordList {
	try {
		$local:keywordNames = (Get-Content $script:wordFilePath -Encoding UTF8 `
			| Where-Object { !($_ -match '^\s*$') } `
			| Where-Object { !($_ -match '^#.*$') }) `
			-as [string[]]
...
function downloadTVerVideo ($script:keywordName, $script:videoPageURL, $script:videoLink) {
	...
	try {
		#ロックファイルをロック
		while ($(fileLock ($script:kFilePath)).fileLocked -ne $true) {
			Write-ColorOutput 'ファイルのロック解除待ち中です'
			Start-Sleep -Seconds 1
		}
		#ファイル操作
		$local:listMatch = Import-Csv $script:tFilePath -Encoding UTF8 `
		| Where-Object { $_.videoPage -eq $script:videoPageURL }
	} catch { Write-ColorOutput 'リストを読み書きできなかったのでスキップしました' Green ; continue
	} finally { $null = fileUnlock ($script:kFilePath) }
	...
	$script:videoFileDir = $(Join-Path $script:nloadBaseDir (getFileNameWithoutInvalidChars $script:videoFileDir))
	$script:videoFilePath = $(Join-Path $script:videoFileDir $script:videoName)
	$script:videoFileRelativePath = $script:videoFilePath.Replace($script:nloadBaseDir, '').Replace('\', '/')
	...
	try {
		...
		#ファイル操作
		$script:newVideo | Export-Csv $script:tFilePath -NoTypeInformation -Encoding UTF8 -Append
		Write-Debug 'リストを書き込みました'
	} catch { Write-ColorOutput 'リストを更新できませんでした。スキップします' Green ; continue
	} finally { $null = fileUnlock ($script:kFilePath) }
	...
	if (($script:ignore -eq $true) -Or ($script:skip -eq $true)) {
		...
		if (-Not (Test-Path $script:videoFileDir -PathType Container)) {
			...
			catch { Write-ColorOutput '保存先ディレクトリが作成できませんでした。スキップします' Green ; continue }
		}
		#youtube-dl起動
